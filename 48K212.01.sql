...
--Loại: Instead of
--Sự kiện: Delete
--Process:	1.Lấy ra MaNhanVien từ bảng deleted ==>@MaNhanVien
			2. Update bảng NhanVien
...
/*
--Bảng: CongViec_BaoCao
--Loại: After 
--Sự kiện: Insert 
--Process:	1. Lấy ra ThoiGianLap,MaCongviec từ bảng Inserted
			==> @ThoiGianLap,@MaCongViec
			2.Lấy ra NgayBatDau và NgayKetThuc từ bảng CongViec
			==>@NgayBatDau, @NgayKetThuc
			3.Kiểm tra sự hợp lệ của @ThoiGianLap (phải nằm trong @NgayBatDau và @NgayKetThuc)
			3.1.Nếu Thời gian lập không nằm trong @NgayBatDau và @NgayKetThuc ==> print 'Thời gian lập không hợp lệ'+Rollback
			3.2. Ngược lại : Insert dữ liệu vào bảng CongViec_BaoCao
*/
go
...
/*
--Input: Không có 
--Output: Danh sách gồm ( Mã nhân viên, tên nhân viên, ngày công, ngày nghỉ phép, loại nghỉ.)
--Process:	1.	Lấy ra  Mã nhân viên, tên nhân viên, ngày công, ngày nghỉ phép, loại nghỉ từ bảng NhanVien và bảng ChamCong, NghiPhep
				Điều kiện ChamCong.NgayCong=NghiPhep.NgayNghi
*/
go
...
/*
--Input: Mã phòng ban
--OutPut: Danh sách nahan viên còn làm việc của một phòng ban
--Process:	1.Lấy ra MaPhongBan==>@MaPhongBan
			2. Trả về Danh sách nhân viên một phòng ban với điều kiện 
			NhanVien.MaPhongBan=@MaPhongBan, TrangThaiLamViec=1
*/
go
...
---
/*6. Trả về danh sách nhân viên còn làm việc có cùng chức vụ nếu biết mã chức vụ
--Input: Mã chức vụ 
--Output: Danh sách nhân viên còn làm việc có cùng chức vụ
--Process:	1. Lấy ra MaChucVu===>@MaChucVu
			2. Lấy ra danh sách bao gồm MaNhanVien,Ten,LuongCoBan,TrangThaiLamViec,MaChucVu,MaPhongBan từ bảng NhanVien với điều kiện
			MaChucVu=@MaChucVu và TrangThaiLamViec=1
*/
go
...
---
Create function Tinhmanhanvien()
returns char(6)
as
begin
	Declare @manvmax char(6),@manvtt char(6),@manvmoi char(6)
	Select top 1 @manvmax=MaNhanVien from NhanVien
	Order by MaNhanVien desc
	Set @manvtt=Isnull(Cast(Right(@manvmax,4)as int),0)+1
	Set @manvmoi= 'NV'+ Format(@manvtt,'0000')
	Return @manvmoi
end
--	f. Thêm mới bản ghi vào bảng NhanVien
...
Create function TinhmaCongmoi()
returns char(11)
as
begin
    Declare @Macongmax char(11), @MaCongtt int, @MaCongmoi char(11)
    Select top 1 @Macongmax = MaCong from ChamCong
    Order by MaCong desc
    Set @MaCongtt = Isnull(Cast(Right(@Macongmax, 9) as int), 0) + 1
    Set @MaCongmoi = 'MC' + Format(@MaCongtt, '000000000')
    Return @MaCongmoi
end

-- Test the function
Select dbo.TinhmaCongmoi() as TinhMaCongMoi
...
--Input: Thang, Nam
--Output: Tổng ngày công đạt chuẩn
--Process:	1.Lấy ra Tổng ngày công ==>@TongNgayCong
			1.1 @TongNgayCong=Count(*)from ChamCong join NhanVien với điều kiện: Month(NgayCong)=@Thang và Year(NgayCong)=@Nam và GioVao is not null
			và GioRa is not null và DateDiff(Hour,Giovao,GioRa)>=3 và MaNhanVien=@MaNV
			2.Trả về TongNgayCong
*/
go
...
--Input: MaNhanVien, tháng, năm.
--Output: Tổng ngày công tăng ca
--Process: 1. Thực hiệm đếm tổng ngày công của nhân viên với điều kiện MaNhanVien=@MaNhanVien và 
			MONTH(NgayCong) = @Thang AND
			YEAR(NgayCong) = @Nam AND
			DATEDIFF(HOUR, GioVao, GioRa) >= 3 AND
			GioRa > '19:00:00' 
			2. Trả về tổng số ngày tăng ca của nhân viên 
*/
go
...
GO
--test
Create function TinhSoNgayNghi (@manv char(6),@Thang int, @Nam int)
returns int
as
begin
	Declare @tongsongaynghi int
	Select @tongsongaynghi=Isnull(Count(NgayNghi),0) from NghiPhep
	Where MaNhanVien=@manv and MONTH(NgayNghi)=@Thang AND YEAR(NgayNghi)=@Nam
	Return @tongsongaynghi
end
